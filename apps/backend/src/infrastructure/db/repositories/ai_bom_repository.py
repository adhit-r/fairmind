...
from storage_adapter import storage_adapter_service

class AIBOMRepository:
    """Repository for AI BOM database operations"""
    
    def __init__(self):
        self.storage_adapter = storage_adapter_service
    ...
    async def create_bom_document(self, request: AIBOMRequest) -> AIBOMDocument:
        ...
        try:
            ...
            if self.storage_adapter.is_connected():
                result = self.storage_adapter.client.table("ai_bom_documents").insert(doc_data).execute()
                if not result.data:
            ...
            for comp in request.components:
                ...
                if self.storage_adapter.is_connected():
                    comp_result = self.storage_adapter.client.table("ai_bom_components").insert(comp_data).execute()
                    if comp_result.data:
    ...
    async def get_bom_document(self, bom_id: str) -> Optional[AIBOMDocument]:
        ...
        try:
            if not self.storage_adapter.is_connected():
                return None
            
            # Get document
            doc_result = self.storage_adapter.client.table("ai_bom_documents")\
                .select("*")\
                .eq("id", bom_id)\
                .execute()
            ...
            # Get components
            comp_result = self.storage_adapter.client.table("ai_bom_components")\
                .select("*")\
                .eq("bom_id", bom_id)\
                .execute()
            ...
            # Get analyses
            analysis_result = self.storage_adapter.client.table("ai_bom_analyses")\
                .select("*")\
                .eq("bom_id", bom_id)\
                .execute()
    ...
    async def list_bom_documents(
        ...
        try:
            ...
            # Build query
            query = self.storage_adapter.client.table("ai_bom_documents").select("*")
    ...
    async def update_bom_document(self, bom_id: str, request: AIBOMRequest) -> AIBOMDocument:
        ...
        try:
            ...
            self.storage_adapter.client.table("ai_bom_components")\
                .delete()\
                .eq("bom_id", bom_id)\
                .execute()
    ...
    async def delete_bom_document(self, bom_id: str) -> bool:
        ...
        try:
            ...
            result = self.storage_adapter.client.table("ai_bom_documents")\
                .delete()\
                .eq("id", bom_id)\
                .execute()
    ...
    async def create_analysis(self, bom_id: str, analysis_data: Dict[str, Any]) -> AIBOMAnalysis:
        ...
        try:
            ...
            if self.storage_adapter.is_connected():
                result = self.storage_adapter.client.table("ai_bom_analyses").insert(db_data).execute()
                if not result.data:
