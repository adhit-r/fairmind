-- Migration 007: Organization RBAC Schema (CORRECTED)
-- Date: 2026-03-22
-- Description: Create organization and role-based access control tables for multi-tenant architecture
-- Note: Corrected for VARCHAR(255) user_id compatibility with existing users table
...
CREATE INDEX IF NOT EXISTS idx_org_members_joined_at ON org_members(joined_at);
...
CREATE INDEX IF NOT EXISTS idx_org_roles_name ON org_roles(name);
CREATE INDEX IF NOT EXISTS idx_org_roles_is_system_role ON org_roles(is_system_role);
...
CREATE INDEX IF NOT EXISTS idx_org_audit_logs_org_id ON org_audit_logs(org_id);
